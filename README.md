# NAM-IPAM-ENVIRONMENT-SSI

Synchronization service that updates prefixes in IPAM to match domain
environment from NAM.

## Overview

NAM-IPAM-ENVIRONMENT-SSI automatically updates prefixes in IPAM if the
configurered domain in NAM doesn't match the prefix custom_field 'env'
...
cd dcn-nam-ipam-environment-ssi
...
SSI_NAME: "NAM-IPAM-ENVIRONMENT-SSI" # Service name
...
SPLUNK_SOURCE: "nam-ipam-environment-ssi:qa" # Log source identifier
SPLUNK_SOURCE_TYPE: "nam-ipam-environment-ssi" # Source type
```
...
IPAM_TOKEN: "eyJhbGciOiJIUzI1NiIsInR5cCIasdfXVCJ9.eyJ1c2VySWQiOiIxMjM0NTY3ODkwIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
```
...
export SSI_NAME="NAM-IPAM-ENVIRONMENT-SSI"
...
export SPLUNK_SOURCE="nam-ipam-environment-ssi"
export SPLUNK_SOURCE_TYPE="nam-ipam-environment-ssi"
```
...
│   └── argo-nam-ipam-environment-ssi.yaml.example # Argo CD Application example
├── logs/                     # Log files directory (auto-created)
├── charts/                   # Helm charts
│   └── dcn-nam-ipam-environment-ssi/ # Production Helm chart
...
│   └── nam-ipam-environment-ssi.yaml # Pod deployment
└── ssi/                      # Source code
...
```bash
# Install using Helm
helm install nam-ipam-environment-ssi-prod ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/prod.yaml \
...
- See `charts/nam-ipam-environment-ssi/README.md` for complete documentation

**Argo CD Deployment:**

```bash
# Apply Argo CD Application manifest
kubectl apply -f examples/argo-nam-ipam-environment-ssi.yaml.example
```

For detailed Helm chart usage, configuration options, and examples, see:

- **Helm Chart README**: `charts/dcn-nam-ipam-environment-ssi/README.md`
- **Argo CD Example**: `examples/argo-nam-ipam-environment-ssi.yaml.example`
...
# Build the image
docker build -t nam-ipam-environment-ssi:latest .
...
kubectl apply -f kubernetes/nam-ipam-environment-ssi.yaml

# Check status
kubectl get pods -n ssi
kubectl logs -n ssi nam-ipam-environment-ssi

# Update configuration
kubectl edit configmap nam-ipam-environment-ssi-config -n ssi
kubectl delete pod nam-ipam-environment-ssi -n ssi  # Restart pod
```
...
- **Pod** (`nam-ipam-environment-ssi.yaml`): Main application deployment
...
docker exec nam-ipam-environment-ssi id
# Should output: uid=1993(deno) gid=1993(deno)

# View logs (logs are stored inside container)
docker logs nam-ipam-environment-ssi
docker logs -f nam-ipam-environment-ssi  # Follow logs in real-time
```
...
docker inspect nam-ipam-environment-ssi | grep -A 10 Mounts
...
# - Verify securityContext in nam-ipam-environment-ssi.yaml
# - Ensure ConfigMap and Secret are properly mounted
kubectl describe pod nam-ipam-environment-ssi -n ssi
kubectl logs nam-ipam-environment-ssi -n ssi
```
...
4. **Run with Docker:**
   ...
   docker-compose up -d
   # View logs
   docker logs -f nam-ipam-environment-ssi
   ```
...
- **GitHub Issues**:
  [NorskHelsenett/dcn-nam-ipam-environment-ssi/issues](https://github.com/NorskHelsenett/dcn-nam-ipam-environment-ssi/issues)
