/**
 * NAM-IPAM-ENVIRONMENT-SSI Main Entry Point
...
const start = async (): Promise<void> => {
  try {
    ...
    if (Deno.env.get("CRON_MODE") !== "true") {
      logger.info(
        `nam-ipam-environment-ssi: Initializing worker on ${Deno.hostname()}`,
      );
      await ssiWorker.work();
      logger.debug(
        `nam-ipam-environment-ssi: Waiting to flush logs before exiting.`,
      );
      Deno.exit(0);
    } else {
      logger.info(
        `nam-ipam-environment-ssi: Initializing worker on ${Deno.hostname()} running every ${SSI_INTERVAL} seconds...`,
      );
      await ssiWorker.work();
    ...
    if (error instanceof Error) {
      logger.error(
        `nam-ipam-environment-ssi: Error occurred on ${Deno.hostname()}, ${error.message}`,
        {
          component: "main",
          method: "start",
      ...
      logger.error(
        `nam-ipam-environment-ssi: Unknown error occurred on ${Deno.hostname()}`,
        {
          component: "main",
          method: "start",
          error: error,
