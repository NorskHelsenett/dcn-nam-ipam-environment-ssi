# NAM-IPAM-ENVIRONMENT-SSI Helm Chart
...
| `workspace`    | Workspace identifier | `nam-ipam-environment-ssi` |

### Image Configuration

| Variable           | Description                | Default                                                           |
| ------------------ | -------------------------- | ----------------------------------------------------------------- |
| `image.repository` | Container image repository | `ncr.sky.nhn.no/ghcr/norskhelsenett/dcn-nam-ipam-environment-ssi` |
...
| `integration.splunk.source` | Splunk source identifier | `nam-ipam-environment-ssi:prod` |
...
Creates CronJob: `nam-ipam-environment-ssi-prod` in namespace `ssi`

### QA Deployment

```bash
helm install nam-ipam-environment-ssi-qa ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/qa.yaml \
  --set credentials.namToken="qa-token-here" \
  --set credentials.splunkToken="qa-splunk-token"
  --set credentials.ipamToken="your-ipam-token"
```

Creates CronJob: `nam-ipam-environment-ssi-qa` in namespace `ssi`

### Test Deployment

```bash
helm install nam-ipam-environment-ssi-test ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/test.yaml \
  --set credentials.namToken="test-token-here" \
  --set credentials.splunkToken="test-splunk-token"
  --set credentials.ipamToken="your-ipam-token"
```

Creates CronJob: `nam-ipam-environment-ssi-test` in namespace `ssi`
...
```bash
# Install
helm install nam-ipam-environment-ssi-{infrastructure} ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/{infrastructure}.yaml \
  --set credentials.namToken="token" \
  --set credentials.splunkToken="splunk-token"
  --set credentials.ipamToken="your-ipam-token"

# Upgrade
helm upgrade nam-ipam-environment-ssi-{infrastructure} ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/{infrastructure}.yaml

# Uninstall
helm uninstall nam-ipam-environment-ssi-{infrastructure}

# Template (dry-run)
helm template nam-ipam-environment-ssi-test ./charts/dcn-nam-ipam-environment-ssi \
  -f charts/dcn-nam-ipam-environment-ssi/env/test.yaml \
...
# Validate
helm lint ./charts/dcn-nam-ipam-environment-ssi
...
See `examples/argo-nam-ipam-environment-ssi.yaml.example` for a complete Argo CD
...
kind: Application
metadata:
  name: nam-ipam-environment-ssi-qa #nam-ipam-environment-ssi-<environment>
  namespace: argocd
...
spec:
  ...
  project: default
  source:
    chart: dcn-nam-ipam-environment-ssi
    helm:
      valueFiles:
        - values.yaml
...
```bash
# Production
argocd app create nam-ipam-environment-ssi-prod \
  --repo ncr.sky.nhn.no/ghcr/norskhelsenett/helm \
  --helm-chart dcn-nam-ipam-environment-ssi \
...
# QA 
argocd app create nam-ipam-environment-ssi-qa \
  --repo ncr.sky.nhn.no/ghcr/norskhelsenett/helm \
  --helm-chart dcn-nam-ipam-environment-ssi \
  --dest-server https://kubernetes.default.svc \
  --dest-namespace ssi \
...
- CronJob naming pattern: `nam-ipam-environment-ssi-{infrastructure}`
- ConfigMap naming pattern: `nam-ipam-environment-ssi-{infrastructure}-config`
- Secret naming pattern: `nam-ipam-environment-ssi-{infrastructure}-secrets`
